...
import {$, $$, create, serializeTags, deserializeTags, GM_setValue, GM_getValue, getBooruParam} from './util';
...
class TagEditor {
  ...
  saveTags(): void {
    ...
    GM_setValue<string[]>(this.id, this.tags);
    this.plainEditor.value = serializeTags(this.tags);
  }

  loadTags(): void {
    this.clearFancyEditor();
    this.tags = GM_getValue<string[]>(this.id, []);
